...
from Components.config import config, ConfigSubsection, ConfigYesNo, ConfigLocations, ConfigText, getConfigListEntry, ConfigInteger
...
from Tools.Directories import fileExists, pathExists, resolveFilename, SCOPE_SKIN, SCOPE_SYSETC
...
functionfile = '/usr/lib/enigma2/python/Plugins/Extensions/spaQButton/mqbfunctions.xml'
config.plugins.spaQButton = ConfigSubsection()
config.plugins.spaQButton.enable = ConfigYesNo(default=True)
config.plugins.spaQButton.info = ConfigYesNo(default=True)
config.plugins.spaQButton.okexitstate = ConfigYesNo(default=False)
config.plugins.spaQButton.mainmenu = ConfigYesNo(default=False)
config.plugins.spaQButton.ac3offauto = ConfigYesNo(default=False)
config.plugins.spaQButton.last_backupdir = ConfigText(default=resolveFilename(SCOPE_SYSETC))
config.plugins.spaQButton.backupdirs = ConfigLocations(default=[resolveFilename(SCOPE_SYSETC)])
config.plugins.spaQButton.channel1 = ConfigInteger(default=1, limits=(0, 9999))
config.plugins.spaQButton.channel2 = ConfigInteger(default=2, limits=(0, 9999))
config.plugins.spaQButton.channel3 = ConfigInteger(default=3, limits=(0, 9999))
config.plugins.spaQButton.channel4 = ConfigInteger(default=4, limits=(0, 9999))
config.plugins.spaQButton.channel5 = ConfigInteger(default=5, limits=(0, 9999))
config.plugins.spaQButton.macroI = ConfigText(default='')
config.plugins.spaQButton.macroII = ConfigText(default='')
config.plugins.spaQButton.macroIII = ConfigText(default='')
config.plugins.spaQButton.macroIV = ConfigText(default='')
config.plugins.spaQButton.macroV = ConfigText(default='')
spaQButton_version = '3.0.01'
...
class spaQButton(Screen, HelpableScreen):
    ...
    def __init__(self, session, args = None, mostrarayuda = False):
        ...
        self.textitul = _('Remap Keys (v') + spaQButton_version + ')'
        self.list = []
    ...
    def opciones(self):
        ...
        listares.append((self.textoac3, 'ac3onoff'))
    ...
    def cbopciones(self, answer):
        ...
        if answer:
            if answer == 'infoKey':
                ...
                self.restoreMenu()
            elif answer == 'ac3onoff':
                self.ac3onoff()

    def ac3onoff(self):
        config.plugins.spaQButton.ac3offauto.value = not config.plugins.spaQButton.ac3offauto.value
        config.plugins.spaQButton.save()
        chekaac3()
        self.updateSettings()
        downmix = {True: _('Enabled'),
         False: _('Disabled')}
        txtdownmix = {True: _('Auto AC3/DTS downmix are activated in standby'),
         False: _('Auto AC3/DTS downmix are NOT activated in standby')}
        dei = self.session.open(MessageBox, _('Dolby Digital downmix') + ' [' + downmix[config.plugins.spaQButton.ac3offauto.value] + ']\n' + txtdownmix[config.plugins.spaQButton.ac3offauto.value], type=MessageBox.TYPE_INFO, timeout=9)
        dei.setTitle(_('AC3/DTS downmix on/off'))
    ...
    def createList(self):
        ...
        for button in values:
            if config.plugins.spaQButton.info.value:
                try:
                    functionbutton = ' ['
                    path = '/etc/MultiQuickButton/quickbutton_' + button + '.xml'
                    if not os_path.exists(path):
                        os_system("cp '/usr/lib/enigma2/python/Plugins/Extensions/spaQButton/quickbutton_.xml' '" + path + "'")
        ...
        self.list.append(QuickButtonListEntry('', (_('Subtitles') + '', 'subtitle'), 'subtitle', functionbutton_subtitle))
        ...
        self.list.append(QuickButtonListEntry('', (_('Program') + '', 'program'), 'program', functionbutton_program))
        self.list.append(QuickButtonListEntry('', (_('Program') + _(' long') + '', 'program_long'), 'program', functionbutton_program_long))
        self.list.append(QuickButtonListEntry('', (_('File') + '', 'file'), 'file', functionbutton_file))
        self.list.append(QuickButtonListEntry('', (_('File') + _(' long') + '', 'file_long'), 'file', functionbutton_file_long))
        self.list.append(QuickButtonListEntry('', (_('List') + '', 'list'), 'list', functionbutton_list))
        self.list.append(QuickButtonListEntry('', (_('List') + _(' long') + '', 'list_long'), 'list', functionbutton_list_long))
        self.list.append(QuickButtonListEntry('', (_('Favorites') + '', 'favorites'), 'favorites', functionbutton_favorites))
        self.list.append(QuickButtonListEntry('', (_('Aspect') + '', 'screen'), 'screen', functionbutton_screen))
        self.list.append(QuickButtonListEntry('', (_('EPG') + '', 'epg'), 'epg', functionbutton_epg))
        self.list.append(QuickButtonListEntry('', (_('EPG') + _(' long') + '', 'epg_long'), 'epg', functionbutton_epg_long))
        self.list.append(QuickButtonListEntry('', (_('History') + '', 'history'), 'history', functionbutton_history))
        self.list.append(QuickButtonListEntry('', (_('History') + _(' long') + '', 'history_long'), 'history', functionbutton_history_long))
        self.list.append(QuickButtonListEntry('', (_('F1') + '', 'f1'), 'f1', functionbutton_f1))
        self.list.append(QuickButtonListEntry('', (_('F1') + _(' long') + '', 'f1_long'), 'f1', functionbutton_f1_long))
        self.list.append(QuickButtonListEntry('', (_('F2') + '', 'f2'), 'f2', functionbutton_f2))
        self.list.append(QuickButtonListEntry('', (_('F2') + _(' long') + '', 'f2_long'), 'f2', functionbutton_f2_long))
        self.list.append(QuickButtonListEntry('', (_('F3') + '', 'f3'), 'f3', functionbutton_f3))
        self.list.append(QuickButtonListEntry('', (_('F3') + _(' long') + '', 'f3_long'), 'f3', functionbutton_f3_long))
        self.list.append(QuickButtonListEntry('', (_('F4') + '', 'f4'), 'f4', functionbutton_f4))
        self.list.append(QuickButtonListEntry('', (_('F4') + _(' long') + '', 'f4_long'), 'f4', functionbutton_f4_long))
        hacciones = {}
        hacciones2 = {}
        lalistadic = []
        lalistadic2 = []
        ...
        for iji in self.list:
            ...
            if _('Not configured') not in valorkey:
                ...
                self['helpbuttonactions' + nombrekey] = myHelpableActionMap(self, 'QuickButtonActions', hacciones, -2)
    ...
    def updateSettings(self):
        ...
        if config.plugins.spaQButton.mainmenu.value:
        ...
        if config.plugins.spaQButton.okexitstate.value:
            ...
            okexit_state += _('off')
        self.textoac3 = _('Auto AC3/DTS downmix on in standby') + ': '
        if config.plugins.spaQButton.ac3offauto.value:
            self.textoac3 += ' ' + _('on')
        else:
            self.textoac3 += ' ' + _('off')
        ...
        self['titul'].setText(self.textitul)
        self.iniciadoS = True
    ...
    def backup(self):
        self.session.openWithCallback(self.callBackup, BackupLocationBox, _('Please select the backup path...'), '', config.plugins.spaQButton.last_backupdir.value)

    def callBackup(self, path):
        if path is not None:
            if pathExists(path):
                config.plugins.spaQButton.last_backupdir.value = path
                config.plugins.spaQButton.last_backupdir.save()
    ...
    def cbdefault(self, resp):
        if resp:
            os_system('tar -xzvf /usr/lib/enigma2/python/Plugins/Extensions/spaQButton/ini/init_etc.rar.gz -C /')
            self.session.open(MessageBox, _('Default settings restored!'), type=MessageBox.TYPE_INFO, timeout=5)
            self.updateList()

    def restore(self):
        self.session.openWithCallback(self.callRestore, BackupLocationBox, _('Please select the restore path...'), '', config.plugins.spaQButton.last_backupdir.value)
    ...
    def autostart(self):
        if config.plugins.spaQButton.enable.value:
            config.plugins.spaQButton.enable.setValue(False)
            config.plugins.spaQButton.mainmenu.setValue(False)
            mens = _('Restarting Enigma2 to unset\nMulti QuickButton Autostart')
        else:
            config.plugins.spaQButton.enable.setValue(True)
            mens = _('Restarting Enigma2 to set\nMulti QuickButton Autostart')
        self.updateSettings()
        config.plugins.spaQButton.enable.save()
    ...
    def setMainMenu(self):
        ...
        if config.plugins.spaQButton.mainmenu.value:
            ...
            config.plugins.spaQButton.mainmenu.setValue(True)
        config.plugins.spaQButton.mainmenu.save()
        self.updateSettings()
    ...
    def setInfo(self):
        return
        if config.plugins.spaQButton.info.value:
            config.plugins.spaQButton.info.setValue(False)
        else:
            config.plugins.spaQButton.info.setValue(True)
        config.plugins.spaQButton.info.save()
        self.updateList()
        self.updateSettings()

    def toggleOkExit(self):
        self.mqbkeymapfile = '/usr/lib/enigma2/python/Plugins/Extensions/spaQButton/keymap.xml'
        ...
        if config.plugins.spaQButton.okexitstate.value:
            config.plugins.spaQButton.okexitstate.setValue(False)
            ...
            config.plugins.spaQButton.okexitstate.setValue(True)
            for self.key in self.keys:
                self.keyinactive = '<!-- ' + self.key + ' -->'
...
class BackupLocationBox(LocationBox):
    ...
    def __init__(self, session, text, filename, dir, minFree = None):
        inhibitDirs = ['/bin',
         ...
         '/usr',
         '/var']
        LocationBox.__init__(self, session, text=text, filename=filename, currDir=dir, bookmarks=config.plugins.spaQButton.backupdirs, autoAdd=True, editDir=True, inhibitDirs=inhibitDirs, minFree=minFree)
    ...
    def QuickPluginSelected(self, choice):
        if choice:
            ...
            for newEntry in self.XML_db.getMenu():
                if newEntry[1] == '1':
                    list.append(QuickButtonListEntry('green', (_(newEntry[0]), newEntry[0], '1')))
                else:
                    list.append(QuickButtonListEntry('red', (_(newEntry[0]), newEntry[0], '')))
...
class spaQButtonChannelConfiguration(Screen, ConfigListScreen):
    ...
    def createConfigList(self):
        self.list = []
        self.list.append(getConfigListEntry(_('Channel') + ' 1', config.plugins.spaQButton.channel1))
        self.list.append(getConfigListEntry(_('Channel') + ' 2', config.plugins.spaQButton.channel2))
        self.list.append(getConfigListEntry(_('Channel') + ' 3', config.plugins.spaQButton.channel3))
        self.list.append(getConfigListEntry(_('Channel') + ' 4', config.plugins.spaQButton.channel4))
        self.list.append(getConfigListEntry(_('Channel') + ' 5', config.plugins.spaQButton.channel5))
    ...
    def keyOk(self):
        ...
        if cur:
            ...
            if self.configmacro == False and self.selectmacro == True:
                ...
                if cur[1] == 'macroI':
                    keys = config.plugins.spaQButton.macroI.value
                elif cur[1] == 'macroII':
                    keys = config.plugins.spaQButton.macroII.value
                elif cur[1] == 'macroIII':
                    keys = config.plugins.spaQButton.macroIII.value
                elif cur[1] == 'macroIV':
                    keys = config.plugins.spaQButton.macroIV.value
                elif cur[1] == 'macroV':
                    keys = config.plugins.spaQButton.macroV.value
    ...
    def save(self):
        if self.configmacro == True and self.selectmacro == False and self.addkey == False:
            ...
            if self.current_macro == 'macroI':
                config.plugins.spaQButton.macroI.value = self.new_config_value
                config.plugins.spaQButton.macroI.save()
            elif self.current_macro == 'macroII':
                config.plugins.spaQButton.macroII.value = self.new_config_value
                config.plugins.spaQButton.macroII.save()
            elif self.current_macro == 'macroIII':
                config.plugins.spaQButton.macroIII.value = self.new_config_value
                config.plugins.spaQButton.macroIII.save()
            elif self.current_macro == 'macroIV':
                config.plugins.spaQButton.macroIV.value = self.new_config_value
                config.plugins.spaQButton.macroIV.save()
            elif self.current_macro == 'macroV':
                config.plugins.spaQButton.macroV.value = self.new_config_value
                config.plugins.spaQButton.macroV.save()
...
class capturatecla(Screen):
    if esHD():
        skin = '\n\t\t<screen position="center,center" size="975,570" title="Capture Key" >\n\t\t<ePixmap name="tecla" position="15,0" size="72,93" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/spaQButton/img/hdteclafs8.png" zPosition="0" alphatest="blend" />\n\t\t<widget name="key_press" position="93,37" size="882,75" halign="left" font="Regular; 20" foregroundColor="#00999999" backgroundColor="#00000000" transparent="1"/>\n\t\t<widget name="key_info" position="15,165" size="945,345" font="Regular; 18" valign="top" halign="left" transparent="1" backgroundColor="#00000000" itemHeight="42"/>\n\t\t<widget name="key_tecla" position="0,525" size="975,45" font="Regular; 21" halign="center" backgroundColor="#00ffffff" foregroundColor="#00000000" zPosition="2" />\n\t\t</screen>'
    else:
        skin = '\n\t\t<screen position="center,center" size="650,380" title="Capture Key" >\n\t\t<ePixmap name="tecla" position="10,0" size="48,62" pixmap="/usr/lib/enigma2/python/Plugins/Extensions/spaQButton/img/teclafs8.png" zPosition="0" alphatest="blend" />\n\t\t<widget name="key_press" position="62,25" size="588,50" halign="left" font="Regular; 20" foregroundColor="#00999999" backgroundColor="#00000000" transparent="1"/>\n\t\t<widget name="key_info" position="10,110" size="630,230" font="Regular; 18" valign="top" halign="left" transparent="1" backgroundColor="#00000000"/>\n\t\t<widget name="key_tecla" position="0,350" size="650,30" font="Regular; 21" halign="center" backgroundColor="#00ffffff" foregroundColor="#00000000" zPosition="2" />\n\t\t</screen>'
    ...
    def __init__(self, session, args = 0):
        ...
        self['actions'] = capMQBActionMap(['QuickButtonActions'], {'red': self.quickSelectGlobal_,
         ...
         'subtitle': self.quickSelectGlobal_,
         ...
         'record_long': self.quickSelectGlobal_,
         'program': self.quickSelectGlobal_,
         'program_long': self.quickSelectGlobal_,
         'list': self.quickSelectGlobal_,
         'list_long': self.quickSelectGlobal_,
         'file': self.quickSelectGlobal_,
         'file_long': self.quickSelectGlobal_,
         'favorites': self.quickSelectGlobal_,
         'screen': self.quickSelectGlobal_,
         'epg': self.quickSelectGlobal_,
         'epg_long': self.quickSelectGlobal_,
         'history': self.quickSelectGlobal_,
         'history_long': self.quickSelectGlobal_,
         'f1': self.quickSelectGlobal_,
         'f1_long': self.quickSelectGlobal_,
         'f2': self.quickSelectGlobal_,
         'f2_long': self.quickSelectGlobal_,
         'f3': self.quickSelectGlobal_,
         'f3_long': self.quickSelectGlobal_,
         'f4': self.quickSelectGlobal_,
         'f4_long': self.quickSelectGlobal_}, -1)
