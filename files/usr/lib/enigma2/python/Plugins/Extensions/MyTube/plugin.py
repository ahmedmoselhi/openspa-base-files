...
config.plugins.mytube.search.orderBy = ConfigSelection([('relevance', _('Relevance')),
 ('viewCount', _('View Count')),
 ('date', _('Published')),
 ('rating', _('Rating'))], 'relevance')
config.plugins.mytube.search.time = ConfigSelection([('all_time', _('All Time')),
 ('this_month', _('This Month')),
 ('this_week', _('This Week')),
 ('today', _('Today'))], 'all_time')
config.plugins.mytube.search.racy = ConfigSelection([('include', _('Yes')), ('exclude', _('No'))], 'include')
config.plugins.mytube.search.categories = ConfigSelection([(None, _('All')),
 ('1', _('Film & Animation')),
 ('2', _('Autos & Vehicles')),
 ('10', _('Music')),
 ('15', _('Pets & Animals')),
 ...
 ('27', _('Education')),
 ('26', _('Howto & Style')),
 ('28', _('Science & Technology'))], None)
config.plugins.mytube.search.lr = ConfigSelection([(None, _('All')),
 ('au', _('Australia')),
 ('br', _('Brazil')),
 ('ca', _('Canada')),
 ('cz', _('Czech Republic')),
 ...
 ('se', _('Sweden')),
 ('tw', _('Taiwan')),
 ('us', _('United States'))], None)
config.plugins.mytube.search.sortOrder = ConfigSelection([('ascending', _('Ascending')), ('descending', _('Descending'))], 'ascending')
...
config.plugins.mytube.general.startFeed = ConfigSelection([('hd', _('HD videos')),
 ...
 ('my_watch_later', _('My Watch Later')),
 ('my_uploads', _('My Uploads')),
 ('my_likes', _('My videos likes'))], 'top_rated')
config.plugins.mytube.general.on_movie_stop = ConfigSelection(default='ask', choices=[('ask', _('Ask user')),
 ('quit', _('Return to movie list')),
 ('playnext', _('Play next video')),
 ('playagain', _('Play video again'))])
config.plugins.mytube.general.on_movie_EOF = ConfigSelection(default='ask', choices=[('ask', _('Ask user')),
 ('quit', _('Return to movie list')),
 ('playnext', _('Play next video')),
 ('playagain', _('Play video again'))])
config.plugins.mytube.general.on_exit = ConfigSelection(default='ask', choices=[('ask', _('Ask user')), ('quit', _('Return to movie list'))])
...
class MyTubePlayerMainScreen(Screen, ConfigListScreen):
    ...
    def __init__(self, session, l2key):
        ...
        self.picload.setPara((85,
         ...
         1,
         '#00e0e0e0'))
        self['searchactions'] = ActionMap(['ShortcutActions',
         'WizardActions',
         'HelpActions',
         'MediaPlayerActions',
         ...
         'menu': self.handleMenu}, -2)
        self['suggestionactions'] = ActionMap(['ShortcutActions',
         'WizardActions',
         'MediaPlayerActions',
         'HelpActions',
         'DirectionActions',
         ...
         'left': self.keyLeft,
         'right': self.keyRight,
         '0': self.toggleScreenVisibility}, -2)
        self['videoactions'] = ActionMap(['ShortcutActions',
         'WizardActions',
         'MediaPlayerActions',
         ...
         'displayHelp': self.handleHelpWindow,
         'menu': self.handleMenu}, -2)
        self['statusactions'] = ActionMap(['ShortcutActions',
         'WizardActions',
         'HelpActions',
         'MediaPlayerActions'], {'back': self.leavePlayer,
         'red': self.leavePlayer,
         'nextBouquet': self.switchToConfigList,
         'green': self.keyStdFeed,
         'yellow': self.handleHistory,
         'menu': self.handleMenu}, -2)
        self['historyactions'] = ActionMap(['ShortcutActions',
         'WizardActions',
         'MediaPlayerActions',
         'MovieSelectionActions',
         'HelpActions',
    ...
    def getuserData(self):
        if myTubeService.is_auth() is True:
            ...
            if fileExists(self.icon, 'r'):
                ...
                client.downloadPage(img_url, self.icon, agent='Mozilla/5.0 (Windows; U; Windows NT 5.1; de; rv:1.9.0.2) Gecko/2008091620 Firefox/3.0.2').addCallback(self.goticon).addErrback(self.showThumbError)
            return username
    ...
    def handleMenu(self):
        if self.currList == 'configlist' or self.currList == 'status':
            ...
            if not myTubeService.is_auth():
                menulist.extend(((_('Login'), 'login'),))
            if vtype == 'youtube#video':
                menulist.extend(((_('View related videos'), 'related'),))
            if self.ytfeed != 'my_subscriptions' and self.ytfeed != 'my_favorites' and self.ytfeed != 'my_history' and self.ytfeed != 'my_watch_later' and self.ytfeed != 'my_uploads' and self.ytfeed != 'my_likes':
                menulist.extend(((_('View user videos'), 'user_videos'),))
            if myTubeService.is_auth() is True and self.ytfeed != 'my_subscriptions' and self.ytfeed != 'my_favorites' and self.ytfeed != 'my_history' and self.ytfeed != 'my_watch_later' and self.ytfeed != 'my_uploads' and self.ytfeed != 'my_likes':
                menulist.extend(((_('Subscribe to user'), 'subscribe'),))
                if vtype == 'youtube#video':
                    menulist.extend(((_('Add to favorites'), 'favorite'), (_('Like this video'), 'like'), (_('Dislike this video'), 'dislike')))
            if self.ytfeed == 'my_subscriptions':
                menulist.extend(((_('Remove Subscription'), 'unsubscribe'),))
            if self.ytfeed == 'my_favorites':
                menulist.extend(((_('Remove From Favorites'), 'deletefav'),))
            if config.usage.setup_level.index >= 2:
                if vtype == 'youtube#video':
                    menulist.extend(((_('Download Video'), 'download'),))
                menulist.extend(((_('View active downloads'), 'downview'),))
            self.hideSuggestions()
            self.session.openWithCallback(self.openMenu, ChoiceBox, title=_('Select your choice.'), list=menulist)
    ...
    def handleLeave(self, how):
        ...
        if how == 'ask':
            if self.currList == 'configlist':
                list = ((_('Yes'), 'quit'), (_('No'), 'continue'), (_('No, but switch to video entries.'), 'switch2feed'))
            else:
                list = ((_('Yes'), 'quit'), (_('No'), 'continue'), (_('No, but switch to video search.'), 'switch2search'))
    ...
    def keyRight(self):
        ...
        elif self.currList == 'suggestionslist':
            if config.plugins.mytube.search.searchTerm.value != '':
    ...
    def keyStdFeed(self):
        ...
        menulist = []
        if myTubeService.is_auth() is True:
            menulist.extend(((_('My Subscriptions'), 'my_subscriptions'),
        ...
        menulist.extend(((_('Top rated'), 'top_rated'),
    ...
    def searchFeed(self, searchContext, vals = None):
        ...
        defaults = {'time': config.plugins.mytube.search.time.value,
        ...
        self.queryStarted()
        self.appendEntries = False
        self.queryThread = myTubeService.search(searchContext, orderby=defaults['orderby'], time=defaults['time'], maxResults=defaults['maxResults'], startIndex=defaults['startIndex'], lr=config.plugins.mytube.search.lr.value, categories=[config.plugins.mytube.search.categories.value], sortOrder=config.plugins.mytube.search.sortOrder.value, callback=self.gotSearchFeed, errorback=self.gotSearchFeedError)
        ...
        for entry in self.screenshotList:
            ...
            if self.Details.has_key(tubeid):
                if self.Details[tubeid]['thumbnail'] is None:
                    ...
                    elif tubeid not in self.pixmaps_to_load:
                        self.pixmaps_to_load.append(tubeid)
    ...
    def fetchFinished(self, x, tubeid, failed = False):
        ...
        if os_path.exists(thumbnailFile) == True:
            ...
            self.picloads[tubeid].setPara((self['thumbnail'].instance.size().width(),
...
class MyTubeVideoInfoScreen(Screen):
    ...
    def __init__(self, session, plugin_path, videoinfo = None):
        ...
        self['shortcuts'] = ActionMap(['ShortcutActions',
         'WizardActions',
         'MovieSelectionActions',
         'DirectionActions'], {'back': self.close,
         'red': self.close,
         'up': self.pageUp,
         'down': self.pageDown,
         'left': self.pageUp,
        ...
        for entry in self.thumbnails:
            if entry[0] == index:
                ...
                if os_path.exists(thumbnailFile) == True:
                    ...
                    self.picloads[index].setPara((self['thumbnail'].instance.size().width(),
                     self['thumbnail'].instance.size().height(),
                     sc[0],
                     sc[1],
                     False,
...
class MyTubeVideoHelpScreen(Screen):
    ...
    def __init__(self, session, plugin_path, wantedinfo = None, wantedtitle = None):
        ...
        self['key_red'] = Button(_('Close'))
        self['detailtext'] = ScrollLabel()
        self['shortcuts'] = ActionMap(['ShortcutActions', 'WizardActions', 'DirectionActions'], {'back': self.close,
         'red': self.close,
         'up': self.pageUp,
         'down': self.pageDown,
         'left': self.pageUp,
...
class MyTubePlayer(Screen, InfoBarNotifications, InfoBarSeek):
    ...
    def keyright(self):
        try:
            if fileExists('/usr/lib/enigma2/python/Plugins/Extensions/TimeSleep/plugin.pyo') and config.plugins.TimeSleep.activar.value:
...
def Plugins(path, **kwargs):
    global plugin_path
    plugin_path = path
    return PluginDescriptor(name=_('My TubePlayer'), description=_('Play YouTube movies'), where=[PluginDescriptor.WHERE_EXTENSIONSMENU, PluginDescriptor.WHERE_PLUGINMENU], icon='plugin.png', fnc=MyTubeMain)
