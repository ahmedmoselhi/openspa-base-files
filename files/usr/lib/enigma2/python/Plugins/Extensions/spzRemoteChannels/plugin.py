...
from EPGImport import EPGdownload
...
config.plugins.RemoteStreamConverter.ip = ConfigIP(default=[0,
 0,
 0,
 0])
...
class ServerEditor(ConfigListScreen, Screen):
    ...
    def __init__(self, session):
        ...
        if config.plugins.RemoteStreamConverter.address.value != '':
            ...
            self.createMenuIp()
        self['actions'] = ActionMap(['SetupActions', 'ColorActions', 'DirectionActions'], {'up': self.keyUp,
         'down': self.keyDown,
         'ok': self.keySave,
         'save': self.keySave,
         'cancel': self.keyCancel,
         'blue': self.enterUrl,
         'yellow': self.switchMode}, -2)
                ...
                try:
                    ...
                    for line in lines:
                        ...
                        if 'http' in line:
                            ...
                            out = '#SERVICE ' + tag2 + ':' + quote('http://' + self.getRemoteAdress() + ':8001/' + tag) + ':' + service_ref.getServiceName() + '\n'
                        else:
                            out = line
                        fp.write(out)
...
def mainInMenu(menuid, **kwargs):
    if menuid == 'scan':
        return [(_('Append Remote channels streams'),
          main,
          'streamconvert',
          99)]
    else:
...
def Plugins(**kwargs):
    return [PluginDescriptor(name=_('Append Remote channels streams'), description=_('Convert remote channel list for streaming'), where=PluginDescriptor.WHERE_MENU, fnc=mainInMenu), PluginDescriptor(name=_('Download EPG from Remote Channels'), description=_('Download EPG from Remote Channels'), where=[PluginDescriptor.WHERE_EXTENSIONSMENU], fnc=Download_EPG)]
