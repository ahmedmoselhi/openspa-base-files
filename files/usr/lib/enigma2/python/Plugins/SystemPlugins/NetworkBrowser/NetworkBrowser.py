...
class NetworkBrowser(Screen):
    ...
    def getNetworkShares(self, hostip, hostname, devicetype):
        ...
        if len(sharelist) == 0:
            ...
            cmd = 'showmount -e {0}'.format(hostip).split()
            try:
                p = subprocess.Popen(cmd, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
                out, err = p.communicate()
                for line in out.split('\n'):
    ...
    def BuildNetworkShareEntry(self, share):
        ...
        elif os_path.exists(resolveFilename(SCOPE_ACTIVE_SKIN, 'networkbrowser/i-smb.png')):
        ...
        for sharename, sharedata in self.mounts.items():
            if sharedata['ip'] == sharehost:
                if sharetype == 'nfsShare' and sharedata['mounttype'] == 'nfs':
                    ...
                    if sharedir == sharedata['sharedir'].replace('/', ''):
        ...
        elif os_path.exists(resolveFilename(SCOPE_ACTIVE_SKIN, 'networkbrowser/cancel.png')):
    ...
    def openMountEdit(self, selection):
        if selection is not None and len(selection):
            ...
            if selection[0] == 'nfsShare':
                ...
                data['active'] = True
                data['ip'] = selection[2]
                data['sharename'] = selection[4].split('/')[-1] + '_' + selection[1]
            ...
            if selection[0] == 'smbShare':
                ...
                data['active'] = True
                data['ip'] = selection[2]
                data['sharename'] = selection[3] + '_' + selection[1]
