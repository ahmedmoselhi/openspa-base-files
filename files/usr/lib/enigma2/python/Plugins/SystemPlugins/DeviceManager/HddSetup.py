...
def DiskEntry(model, size, removable, device):
    if device[:3] == 'mmc':
        picture = LoadPixmap(cached=True, path=resolveFilename(SCOPE_CURRENT_PLUGIN, 'SystemPlugins/DeviceManager/icons/disksd.png'))
    elif removable:
...
class HddSetup(Screen):
    ...
    def __init__(self, session, args = 0):
        ...
        for disk in self.mdisks.disks:
            ...
            self.disks.append(DiskEntry(disk[3], capacity, disk[2], disk[0]))
