...
class HddMount(Screen):
    ...
    def setMountPoint(self, path):
        ...
        if self.mountpoints.exist(path):
            self.session.openWithCallback(self.setMountPointCb, ExtraMessageBox, _('Selected mount point is already used by another drive.'), _('Mount point exist!'), [[_('Change old drive with this new drive'), 'ok.png'], [_('Keep old drive'), 'cancel.png']])
        else:
...
class HddFastRemove(Screen):
    if esHD():
        skin = '\n\t\t<screen name="HddFastRemove" position="center,center" size="840,645" title="Hard Drive Fast Umount">\n\t\t<ePixmap pixmap="usr/lib/enigma2/python/Plugins/Extensions/spazeMenu/imgvk/hdred.png" position="0,0" size="210,60" alphatest="blend" />\n\t\t<ePixmap pixmap="usr/lib/enigma2/python/Plugins/Extensions/spazeMenu/imgvk/hdblue.png" position="210,0" size="210,60" alphatest="blend" />\n\t\t<widget name="key_red" position="0,0" zPosition="1" size="210,60" font="Regular;20" halign="center" valign="center" backgroundColor="#9f1313" transparent="1" />\n\t\t<widget name="key_blue" position="210,0" zPosition="1" size="210,60" font="Regular;20" halign="center" valign="center" backgroundColor="#18188b" transparent="1" />\n\t\t<widget source="menu" render="Listbox" position="15,82" size="780,570" scrollbarMode="showOnDemand" itemHeight="42">\n\t\t\t<convert type="TemplatedMultiContent">\n\t\t\t\t{"template": [\n\t\t\t\t\tMultiContentEntryPixmapAlphaTest(pos = (7,0), size = (72,72), png = 0),\n\t\t\t\t\tMultiContentEntryText(pos = (97,4), size = (285,57), font=0, flags = RT_HALIGN_LEFT|RT_VALIGN_TOP, text = 1),\n\t\t\t\t\tMultiContentEntryText(pos = (247,40), size = (435,57), font=1, flags = RT_HALIGN_LEFT|RT_VALIGN_TOP, text = 2),\n\t\t\t\t\t],\n\t\t\t\t\t"fonts": [gFont("Regular", 22), gFont("Regular", 18)],\n\t\t\t\t\t"itemHeight": 75\n\t\t\t\t}\n\t\t\t</convert>\n\t\t</widget>\n\t\t</screen>'
    else:
