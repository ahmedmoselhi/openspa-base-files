...
class MountPoints:
    ...
    def getRealMount(self, device, partition):
        ...
        for mount in mounts:
            res = mount.split(' ')
            if res and len(res) > 1:
                if res[0] == '/dev/%s%i' % (device, partition) or res[0] == '/dev/%sp%i' % (device, partition):
                    mounts.close()
                    return res[1]
    ...
    def add(self, device, partition, path):
        ...
        for entry in self.entries:
            if len(entry['data']) == 6:
                if entry['data'][0] == '/dev/%s%i' % (device, partition) or entry['data'][0] == '/dev/%sp%i' % (device, partition):
                    self.entries.remove(entry)
    ...
    def getUUID(self, device, partition):
        ...
        for row in rows:
            ...
            if len(tmp) < 2:
                continue
            if tmp[0] == '/dev/%s%i' % (device, partition) or tmp[0] == '/dev/%sp%i' % (device, partition):
                tmp.reverse()
                key = tmp.pop()
                tmp.reverse()
                value = ':'.join(tmp)
                ...
                if ret:
                    uuid = ret.group(1)
                self.uuids.append({'device': device,
                 'partition': partition,
                 'uuid': uuid})
                return uuid
