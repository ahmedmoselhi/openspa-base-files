...
def ruticon(carpeta):
    ...
    if esHD():
        anadirruta = 'HD'
    rutaicono = 'SystemPlugins/DeviceManager/' + carpeta + anadirruta + '/'
...
class HddPartitions(Screen):
    ...
    def __init__(self, session, disk):
        ...
        if len(self.disk[5]) > 0:
            ...
            if self.disk[5][0][3] == '83' or self.disk[5][0][3] == '7' or self.disk[5][0][3] == 'b' or self.disk[5][0][3] == '6':
    ...
    def selectionChanged(self):
        ...
        if len(self.disk[5]) > 0:
            ...
            if self.disk[5][index][3] == '83' or self.disk[5][index][3] == '7' or self.disk[5][index][3] == 'b':
                self['key_green'].setText(_('Check'))
                self['key_yellow'].setText(_('Format'))
            if self.disk[5][index][3] == '83' or self.disk[5][index][3] == '7' or self.disk[5][index][3] == 'b' or self.disk[5][index][3] == '6':
    ...
    def chkfs(self):
        disks = Disks()
        if len(self.disk[5][self.index][0]) > 6:
            ret = disks.chkfs(self.disk[5][self.index][0][:8], self.index + 1, self.fstype)
        else:
    ...
    def mkfs(self):
        disks = Disks()
        if len(self.disk[5][self.index][0]) > 6:
            ret = disks.mkfs(self.disk[5][self.index][0][:8], self.index + 1, self.fstype)
        else:
    ...
    def red(self):
        if len(self.disk[5]) > 0:
            index = self['menu'].getIndex()
            if self.disk[5][index][3] != '83' and self.disk[5][index][3] != '7' and self.disk[5][index][3] != 'b' and self.disk[5][index][3] != '6':
                return
